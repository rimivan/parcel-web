...
const createTextElement = async () => { 
  const resolvedData = await getTextData(); /* without async/await, Promise returned by getTextData cannot be resolved */
  const body = document.querySelector("body");
	const divElement = document.createElement("div");
  
  const textNode = document.createTextNode(resolvedData);
